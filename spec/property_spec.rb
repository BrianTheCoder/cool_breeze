...
describe "Properties" do
  before(:each) do
    reset_test_db!
    @user = User.new(:first_name => "matt")
  end
  
  it "should be accessible from the object" do
    @user.properties.should be_an_instance_of(Array)
    @user.properties.map{|p| p.name}.should include("first_name")
  end
  
  it "should let you access a property value (getter)" do
    @user.first_name.should == "matt"
  end
  
  it "should let you set a property value (setter)" do
    @user.last_name = "Aimonetti"
    @user.last_name.should == "Aimonetti"
  end
  
  # it "should not let you set a property value if it's read only" do
  #   lambda{@user.read_only_value = "test"}.should raise_error
  # end
  ...
  #     @user.errors.on(:first_name).should == ["First name must not be blank"]
  #   end
  # 
